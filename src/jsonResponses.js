...
const getType = (request, response) => {
  ...
  const pokemon = pokedex.filter(
    (poke) => poke.type.some((t) => t.toLowerCase() === type.toLowerCase()),
  );
...
const getFinalStageEvolution = (request, response) => {
  const finalStagePokemon = pokedex.filter((poke) => !poke.next_evolution);
...
const getRandomPokemon = (request, response) => {
  ...
  const { numPokemon } = request.query;
...
const editPokemon = (request, response) => {
  ...
  request.on('end', () => {
    ...
    const index = pokedex.findIndex(
      (p) => p.name.toLowerCase() === updatedPokemon.name.toLowerCase(),
    );
